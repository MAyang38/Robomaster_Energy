...
int main() {
    ...
    dim3 blockPerGrid((IMAGE_COLS + threadsPerBlock.x - 1) / threadsPerBlock.x,
                      (IMAGE_ROWS + threadsPerBlock.y - 1) / threadsPerBlock.y);
    ...
    while (true) {
        ...
        cudaMemcpy(binarization_cuda_sobel.data, sobel_img_gpu, IMAGE_ROWS * IMAGE_COLS * sizeof(unsigned char),
                   cudaMemcpyDeviceToHost);

        scharr_gpu(gray_gpu, scharr_img_gpu, IMAGE_ROWS, IMAGE_COLS, threadsPerBlock, blockPerGrid);
        cudaMemcpy(binarization_cuda_scharr.data, scharr_img_gpu, IMAGE_ROWS * IMAGE_COLS * sizeof(unsigned char),
                   cudaMemcpyDeviceToHost);
