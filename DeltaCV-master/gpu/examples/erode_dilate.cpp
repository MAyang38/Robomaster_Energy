...
int main() {
    ...
    while (true) {
        ...
        cudaMemcpy(binarization_cuda_erode.data, erode_img_gpu, IMAGE_ROWS * IMAGE_COLS * sizeof(unsigned char),
                   cudaMemcpyDeviceToHost);

        dilate_gpu(gray_gpu, dilate_img_gpu, IMAGE_ROWS, IMAGE_COLS, cv::Size(5, 5), threadsPerBlock, blockPerGrid);
        cudaMemcpy(binarization_cuda_dilate.data, dilate_img_gpu, IMAGE_ROWS * IMAGE_COLS * sizeof(unsigned char),
                   cudaMemcpyDeviceToHost);
