...
namespace {
    ...
    void decomposeHomography(const string &img1Path, const string &img2Path, const Size &patternSize,
        ...
        bool found1 = findChessboardCorners(img1, patternSize, corners1);//!检测棋盘图角点
        ...
        //! [compute-poses]
        vector<Point3f> objectPoints;//!3维坐标矩阵
        calcChessboardCorners(patternSize, squareSize, objectPoints);//!寻找棋盘图内角点

        FileStorage fs(intrinsicsPath, FileStorage::READ);//!读取参数
        Mat cameraMatrix, distCoeffs;//!相机内参，外参；由接下来两行代码从参数表中读取
        fs["camera_matrix"] >> cameraMatrix;//读取内参
        fs["distortion_coefficients"] >> distCoeffs;//读取外参

        Mat rvec1, tvec1;//!旋转，平移向量
        ...
        for (int i = 0; i < solutions; i++) {
            ...
            cout << "tvec from homography decomposition: " << ts_decomp[i].t() << " and scaled by d: "
                 << factor_d1 * ts_decomp[i].t() << endl;
